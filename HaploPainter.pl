...
### default values global and family specific
my $def = {
	   GLOB => {
		    BACKGROUND => '#ffffff',
		    BORDER => 20,
...
#=========
	...
	### attach the menu to Main Window
	...
	### navigation menu buttons (ugly cursor shapes due to Tk)
	$b1 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetArrow), -relief => 'sunken',
			  -command => sub {
	...
	$b2 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetPlus), -relief => 'flat',
			  -command => sub {
	...
	$b3 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetMinus), -relief => 'flat',
			  -command => sub {
	...
	$b4 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetHand), -relief => 'flat',
			  -command => sub {
	...
	$b5 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetNextLeft), -relief => 'flat',
			  -command => sub {
				  DrawNextFamily(1);
			  })->pack(qw/-side left/);
	$b6 = $f1->Button(-image => $mw->Photo(-format => 'gif', -data => &GetNextRight), -relief => 'flat',
			  -command => sub {
				  DrawNextFamily(0);
			  })->pack(qw/-side left/);
	...
	$canvas = $f2->Scrolled('Canvas', -width => 10000, -height => 10000,
				-bg => $self->{GLOB}{BACKGROUND}, -scrollbars => 'osoe',
				-scrollregion => [ 0, 0, 2000, 2000 ], -cursor => 'left_ptr')->pack(-expand => 1, -fill => 'both');
	...
	$mw->bind('<Key-F4>' => \&ImportPedigreeDBI);
	...
	### assume the argument is a pedfile to open and draw
...
# build the whole menu stuff at once
#==============
sub _MainMenu {
#==============
	[
			...
			[
				...
				[ 'cascade', 'Import Pedigrees ...', -tearoff => 0, -menuitems =>
					[
						...
						[ 'command', 'Database', -command => \&ImportPedigreeDBI, -accelerator => 'F4' ],
					]
				],
		...
		[ '~Options',
			[
				...
				[ 'command', 'Consanguinity Settings ...', -command => \&OptionsConsanguine ],
				[ 'command', 'Page Settings ...', -command => \&OptionsPrint ],
				'-',
...
# some functions selectable via post menu (right mouse click)
#=================
sub _ContextMenu {
#=================
	[
...
## attach a new core family
#================
sub NewPedigree {
#================
	our $menubar;

	my $d = $mw->DialogBox(-title => 'Add new family', -buttons => [ qw/Ok Cancel/ ]);
	...
	$f1->Entry(-textvariable => \$fam_new, -width => 20)->grid(-row => 0, -column => 1, -sticky => 'w');

	$d->Show eq 'Ok' || return;
	return unless $fam_new;
	return if $self->{FAM}{PED_ORG}{$fam_new};
	$self->{GLOB}{CURR_FAM} = $fam_new;
	$self->{GLOB}{FILENAME} = "new_pedfile.hp" unless $self->{GLOB}{FILENAME};
...
#=================
	...
	my $var;
	if (scalar @list > 1) {
		my $d = $mw->DialogBox(-title => 'Choose family', -buttons => [ qw/Ok Cancel/ ]);
		my $f1 = $d->Frame(-relief => 'groove', -borderwidth => 2)->pack(-padx => 5, -pady => 5, -expand => 1, -fill => 'both', -anchor => 'w');

		$f1->Label(-text => 'Family')->grid(-row => 0, -column => 0, -sticky => 'e');
		$f1->BrowseEntry(-textvariable => \$family, -width => 20, -choices => [ @list ])->grid(-row => 0, -column => 1, -sticky => 'w');
	...
	if ($var) {
		@fam = keys %{$self->{FAM}{PED_ORG}};
	}
	for my $fam (@fam) {
		for my $k (keys %{$def->{FAM}}) {
			if (ref $def->{FAM}{$k}) {
				for my $k2 (keys %{$def->{FAM}{$k}}) {
					$self->{FAM}{$k}{$fam}{$k2} = $test->{FAM}{$k}{$family}{$k2};
				}
			} else {
				$self->{FAM}{$k}{$fam} = $test->{FAM}{$k}{$family};
			}
		}
	}
	RedrawPedForce();
...
### Tk and GTK+ must be present anyway (as libraries and perl bindings)
#=====================
sub ExecuteBatchMode {
#=====================
	my %pok = map { $_ => 1 } qw/-pedfile -pedformat -outfile -outformat -hapfile -hapformat -mapfile -mapformat
				     -family -resolution -paper -orientation -border -bgcolor -dbtype -dbport -dbsid
				     -dbhost -dbtable -dbuname -dbpasswd -breakloop -sortbyid -sortbygender/;
	my @opts = nsort keys %pok;

	if ($ARGV[0] && $ARGV[0] eq '-h') {
		print "Valid parameters are: " . join(", ", @opts) . "\n";
		exit;
	}

	if (!@ARGV || $ARGV[0] ne '-b' || scalar @ARGV < 2) {
		return;
	}
	...
	for my $opt (keys %arg) {
		unless ($pok{$opt}) {
			print "Unknown parameter $opt\nValid parameters are:" . join(", ", @opts) . "\n";
			exit;
		}
	}

	unless ($arg{-family}) {
		print "Need family (-family)!\n";
		exit;
	}

	### resolution
	if ($arg{-resolution}) {
		unless (($arg{-resolution} =~ /^\d{2,4}$/) && ($arg{-resolution} <= 4800)) {
			print "Invalid resolution value (10-4800)\n";
			exit;
		}
	...
	### paper
	if ($arg{-paper}) {
		unless ($param->{PAPER_SIZE}{$arg{-paper}}) {
			print "Unknown paper size $arg{-paper} - must be (" . join(", ", nsort keys %{$param->{PAPER_SIZE}}) . ")\n";
			exit;
		}
	...
	### orientation
	if ($arg{-orientation}) {
		unless (($arg{-orientation} =~ /^landscape$/i) || ($arg{-orientation} =~ /^portrait$/i)) {
			print "Unknown orientation $arg{-orientation} - must be landscape or portrait\n";
			exit;
		}
	...
	### border
	if ($arg{-border}) {
		unless (($arg{-border} =~ /^\d{1,3}$/)) {
			print "Invalid border values (1-999)\n";
			exit;
		}
	...
	if ($arg{-sortbyid}) {
		$param->{SORT_BY_PEDID} = $arg{-sortbyid};
	}
	if ($arg{-sortbygender}) {
		$param->{SORT_COUPLE_BY_GENDER} = $arg{-sortbygender};
	}

	if ($arg{-pedfile} && -f $arg{-pedfile}) {
		### read in pedfile
		if ($arg{-pedformat} && $param->{PEDIGREE_FORMATS}{lc $arg{-pedformat}}) {
			$pedformat = lc $arg{-pedformat};
		}

		ReadPed(-file => $arg{-pedfile}, -format => uc $pedformat) or die "ReadPed";

		unless ($self->{FAM}{PED_ORG}{$arg{-family}}) {
			print "Family $arg{-family} is unknown!\n";
			exit;
		}
		...
		unless ($arg{-dbport} && $arg{-dbport} =~ /^\d{3,5}$/) {
			print "Missing/invalid database port (-dbport)\n";
			exit;
		}
		$self->{GLOB}{DB_PORT} = $arg{-dbport};

		unless ($arg{-dbsid}) {
			print "Missing database name or SID (-dbsid)\n";
			exit;
		}
		$self->{GLOB}{DB_SID} = $arg{-dbsid};

		unless ($arg{-dbhost}) {
			print "Missing database host name/address (-dbhost)\n";
			exit;
		}
		$self->{GLOB}{DB_HOST} = $arg{-dbhost};

		unless ($arg{-dbtable}) {
			print "Missing database table name (-dbtable)\n";
			exit;
		}
		$self->{GLOB}{DB_RELATION} = $arg{-dbtable};

		unless ($arg{-dbuname}) {
			print "Missing database user/login name (-dbuname)\n";
			exit;
		}
		$self->{GLOB}{DB_UNAME} = $arg{-dbuname};

		unless ($arg{-dbpasswd}) {
			print "Missing database password (-dbpasswd)\n";
			exit;
		}
		...
		print "Need pedfile (-pedfile) or database connection (-dbtype, -dbport, -dbsid, -dbhost, -dbtable, -dbuname, -dbpasswd)!\n";
		exit;
	}

	### background color
	if ($arg{-bgcolor}) {
		unless (($arg{-bgcolor} =~ /^\#[0-9a-f]{6}$/)) {
			print "Invalid color format - example is '#ffc000'\n";
			exit;
		}
	...
	unless ($arg{-outfile}) {
		print "Need output file (-outfile)!\n";
		exit;
	}
	unless ($arg{-outformat}) {
		print "Need output format!\n";
		exit;
	}
	unless ($param->{EXPORT_FORMATS}{uc $arg{-outformat}}) {
		print "Unknown output format $arg{-outformat}!\n";
	}

	if ($arg{-hapfile}) {
		unless (-f $arg{-hapfile}) {
			print "Could not open file $arg{-hapfile}!\n";
			exit;
		}
		unless ($arg{-hapformat}) {
			print "Need haplotype format (-hapformat)!\n";
			exit;
		}
		unless ($param->{HAPLOTYPE_FORMATS}{lc $arg{-hapformat}}) {
			print "Unknown haplotype format - $arg{-hapformat} -!\n";
		}
	...
	if ($arg{-mapfile}) {
		 unless (-f $arg{-mapfile}) {
			 print "Could not open file $arg{-mapfile}!\n";
			 exit;
		 }
		 unless ($arg{-mapformat}) {
			 print "Need map format (-mapformat)!\n";
			 exit;
		 }
		 unless ($param->{MAP_FORMATS}{lc $arg{-mapformat}}) {
			 print "Unknown map format $arg{-mapformat}!\n";
		 }
	...
	for my $sym (@act_sym) {
		...
		for my $r (@{$self->{FAM}{PED_ORG}{$fam}}) {
			if ($r && $r->[0] eq $p_old) {
				...
				for my $child_id (keys %{$self->{FAM}{CHILDREN}{$fam}{$p}}) {
					my $child_old = $self->{FAM}{CASE_INFO}{$fam}{PID}{$child_id}{Case_Info_1} or next;
					for my $r2 (@{$self->{FAM}{PED_ORG}{$fam}}) {
...
#===============
	...
	if (keys %t) {
		...
		for my $sib (@sibs) {
			my $sib_old = $ci->{PID}{$sib}{Case_Info_1};
			unless ($self->{FAM}{SID2FATHER}{$fam}{$sib}) {
				ShowInfo("The twin individual $sib_old must not be a founder.", 'warning');
				return;
			}
			$par = $self->{FAM}{SID2FATHER}{$fam}{$sib} . '==' . $self->{FAM}{SID2MOTHER}{$fam}{$sib} unless $par;
			$gender = $self->{FAM}{SID2SEX}{$fam}{$sib} unless $gender;

			### twins should be siblings
			unless ($self->{FAM}{SIBS}{$fam}{$par}{$sib}) {
...
#=====================
	...
	my @act_sym = keys %{$self->{GLOB}{ACTIVE_SYMBOLS}} or return;
	return unless scalar @act_sym == 2;
	...
	unless ($self->{FAM}{COUPLE}{$fam}{$p1} && $self->{FAM}{COUPLE}{$fam}{$p1}{$p2}) {
		ShowInfo("The individuals you selected are no couples!", 'warning');
		return;
	}
	...
	if ($self->{FAM}{CONSANGUINE_MAN}{$fam}{$p1}{$p2}) {
		...
		for (@{$self->{FAM}{PED_ORG}{$fam}}) {
			if (($_->[0] eq $p1_old) || ($_->[0] eq $p2_old)) {
				undef $_->[10];
			}
		}
	} else {
...
#===============
	...
	return unless $self->{FAM}{FOUNDER}{$fam}{$p};

	my ($f, $m);

	my $d = $mw->DialogBox(-title => 'Add parents', -buttons => [ qw/Ok Cancel/ ]);
	my $f1 = $d->Frame(-relief => 'groove', -borderwidth => 2)->pack(-padx => 5, -pady => 5, -expand => 1, -fill => 'both', -anchor => 'w');

	$f1->Label(-text => 'Name of father')->grid(-row => 0, -column => 0, -sticky => 'e');
	$f1->Entry(-textvariable => \$f, -width => 20)->grid(-row => 0, -column => 1, -sticky => 'w');

	$f1->Label(-text => 'Name of mother')->grid(-row => 1, -column => 0, -sticky => 'e');
	$f1->Entry(-textvariable => \$m, -width => 20)->grid(-row => 1, -column => 1, -sticky => 'w');
	...
	push @{$self->{FAM}{PED_ORG}{$fam}}, [ $f, 0, 0, 1, 1 ] unless $self->{FAM}{PID2PIDNEW}{$fam}{$f};
	push @{$self->{FAM}{PED_ORG}{$fam}}, [ $m, 0, 0, 2, 1 ] unless $self->{FAM}{PID2PIDNEW}{$fam}{$m};

	for my $l (@{$self->{FAM}{PED_ORG}{$fam}}) {
		if ($l->[0] eq $p_old) {
			$l->[1] = $f;
			$l->[2] = $m;
		}
...
#========================
	...
	$f1->Entry(-textvariable => \$child, -width => 20)->grid(-row => 1, -column => 1, -sticky => 'w');
	...
	if ($sex_p == 1) {
		push @{$self->{FAM}{PED_ORG}{$fam}}, [ $mate, 0, 0, 2, 1 ] unless $self->{FAM}{PID2PIDNEW}{$fam}{$mate};
		push @{$self->{FAM}{PED_ORG}{$fam}}, [ $child, $pid_old, $mate, 0, 1 ];
	} else {
		push @{$self->{FAM}{PED_ORG}{$fam}}, [ $mate, 0, 0, 1, 1 ] unless $self->{FAM}{PID2PIDNEW}{$fam}{$mate};
...
#================
	...
	for (1 .. 8) {
		...
		$f1->Entry(-textvariable => \$pid_new->{$_}{NAME}, -width => 20)->grid(-row => $_ - 1, -column => 2, -sticky => 'w');
...
#===================
	...
	for my $k1 (keys %{$def->{GLOB}}) {
		if (ref $def->{GLOB}{$k1}) {
			for my $k2 (keys %{$def->{GLOB}{$k1}}) {
				if (ref $def->{GLOB}{$k1}{$k2}) {
					for my $k3 (keys %{$def->{GLOB}{$k1}{$k2}}) {
						$self->{GLOB}{$k1}{$k2}{$k3} = $def->{GLOB}{$k1}{$k2}{$k3};
					}
				} else {
					$self->{GLOB}{$k1}{$k2} = $def->{GLOB}{$k1}{$k2};
				}
			}
		} else {
			$self->{GLOB}{$k1} = $def->{GLOB}{$k1};
		}
	}
}
...
#=================
	...
	## copy up to three hash levels from $def to $self
	for my $k1 (keys %{$def->{FAM}}) {
		if (ref $def->{FAM}{$k1}) {
			for my $k2 (keys %{$def->{FAM}{$k1}}) {
				if (ref $def->{FAM}{$k1}{$k2}) {
					for my $k3 (keys %{$def->{FAM}{$k1}{$k2}}) {
						$self->{FAM}{$k1}{$fam}{$k2}{$k3} = $def->{FAM}{$k1}{$k2}{$k3};
					}
				} else {
					$self->{FAM}{$k1}{$fam}{$k2} = $def->{FAM}{$k1}{$k2};
				}
			}
		} else {
			$self->{FAM}{$k1}{$fam} = $def->{FAM}{$k1};
		}
	}
}
...
#===============
	...
	for ($self->{GLOB}{ACTIVE_ITEM}) {
		...
		if (/ALLEL-(\w)-(\d+)-(.+)/) {
			...
			for my $l ('Paternal', 'Maternal', 'Not informative') {
				$f->Radiobutton(-text => $l, -variable => \$var, -value => $l, -command => sub {
					if ($var eq 'Paternal') {
						$flag = 1;
						$self->{FAM}{HAPLO}{$fam}{PID}{$3}{$1}{BAR}[$2][1] = $P;
						$self->{FAM}{HAPLO}{$fam}{PID}{$3}{$1}{BAR}[$2][0] = 'NI-3';

						if ($self->{FAM}{DUPLICATED_PID}{$fam}{$3}) {
							for my $dupl (keys %{$self->{FAM}{DUPLICATED_PID}{$fam}{$3}}) {
								$self->{FAM}{HAPLO}{$fam}{PID}{$dupl}{$1}{BAR}[$2][1] = $P;
								$flag = 1;
						...
						$flag = 1;
						$self->{FAM}{HAPLO}{$fam}{PID}{$3}{$1}{BAR}[$2][1] = $M;
						$self->{FAM}{HAPLO}{$fam}{PID}{$3}{$1}{BAR}[$2][0] = 'NI-3';

						if ($self->{FAM}{DUPLICATED_PID}{$fam}{$3}) {
							for my $dupl (keys %{$self->{FAM}{DUPLICATED_PID}{$fam}{$3}}) {
								$flag = 1;
								$self->{FAM}{HAPLO}{$fam}{PID}{$dupl}{$1}{BAR}[$2][1] = $M;
								$self->{FAM}{HAPLO}{$fam}{PID}{$dupl}{$1}{BAR}[$2][0] = 'NI-3';
							}
						}
						...
						$flag = 1;
						$self->{FAM}{HAPLO}{$fam}{PID}{$3}{$1}{BAR}[$2][0] = 'NI-2';
						if ($self->{FAM}{DUPLICATED_PID}{$fam}{$3}) {
							for my $dupl (keys %{$self->{FAM}{DUPLICATED_PID}{$fam}{$3}}) {
...
#==============
	...
	$f1->Entry(-textvariable => \$self->{FAM}{TITLE}{$fam}, -width => 20)->grid(-row => 0, -column => 1, -sticky => 'w');
	$d->Show eq 'Ok' || return;
	RedrawPed();
}
...
#================
	...
	for my $tag ($c->itemcget('current', -tags)) {
		if ($tag =~ /SYM-(\S+)/) {
			...
			my $d = $mw->DialogBox(-title => 'Change individual settings', -buttons => [ qw/Ok Cancel/ ]);
			...
			$f1->BrowseEntry(-choices => [ 0 .. 9 ], -state => 'readonly',
					 -textvariable => \$self->{FAM}{SID2AFF}{$fam}{$id}, -width => 3)->grid(-row => 0, -column => 2, -sticky => 'w');
			...
			$f2->Entry(-textvariable => \$self->{FAM}{INNER_SYMBOL_TEXT}{$fam}{$id}, -width => 20)->grid(-row => 0, -column => 1, -sticky => 'w');
			...
			for (1 .. 4) {
				...
				$f4->Entry(-textvariable => \$ci->{$id}{$var}, -width => 25)->grid(-row => $_ - 1, -column => 1, -sticky => 'w');
			...
			if ($_ eq 'Cancel') {
				...
				if ($sid_old ne $sid_old_save) {
					if ($self->{FAM}{PID2PIDNEW}{$fam}{$sid_old}) {
						ShowInfo("SID $sid_old is already in use!", 'warning');
						return;
					}
					...
					for my $l (@{$self->{FAM}{PED_ORG}{$fam}}) {
						next unless $l;
						for (0 .. 2) {
							$l->[$_] = $sid_old if $l->[$_] eq $sid_old_save;
						}
					}
...
#==================
	...
	unless ($self->{GLOB}{STATUS}) {
		...
		if (keys %{$self->{GLOB}{ACTIVE_SYMBOLS}}) {
			...
			for (keys %{$self->{GLOB}{ACTIVE_SYMBOLS}}) {
...
#================
	...
	unless ($self->{GLOB}{STATUS}) {
		...
		if ($curr_tag) {
			...
			for (@act_sym) {
				unless ($self->{GLOB}{ACTIVE_SYMBOLS}{$_}{ID_CLONE}) {
					my $pid = $self->{GLOB}{ACTIVE_SYMBOLS}{$_}{PID} or print "NO PID\n";
					@_ = ($c->coords($_), -width => 1, -outline => 'white', -fill => 'grey85');
					my $id;
					if ($self->{FAM}{IS_SAB_OR_TOP}{$fam}{$pid} || !$self->{FAM}{SID2SEX}{$fam}{$pid}) {
						$id = $c->createPolygon(@_);
					} elsif ($self->{FAM}{SID2SEX}{$fam}{$pid} == 1) {
						$id = $c->createRectangle(@_);
					} elsif ($self->{FAM}{SID2SEX}{$fam}{$pid} == 2) {
						$id = $c->createOval(@_);
					}
...
#=================
	...
	unless ($self->{GLOB}{STATUS}) {
		...
		unless ($self->{GLOB}{ACTIVE_SYMBOLS}{$curr_tag}) {
			for (keys %{$self->{GLOB}{ACTIVE_SYMBOLS}}) {
				/^SYM-(.+)$/;
...
#==============
	...
	SetLines();
	SetHaplo();
	DrawOrExportCanvas() unless $batch;
}
...
#===================
	...
	for my $k (keys %{$self->{FAM}}) {
		next if $k eq 'MAP';
		next if $k eq 'TITLE';
		undef $self->{FAM}{$k}{$fam} unless defined $def->{FAM}{$k};
	}
	undef $self->{GLOB}{ACTIVE_SYMBOLS};
...
#============
	...
	while ($flag) {
		...
		for my $m (1 .. $n) {
			BuildMatrix($fam);
			my $count = 0;
			until (AlignMatrix($fam)) {
				$count++;
				last if $count > $param->{MAX_COUNT};
			}
			SetLines($fam);
			...
			unless ($batch) {
				my $text = sprintf("%9s%18s%20s  ", "Try #$m", "Crosses #$c", "min crosses #$CrossMin");
...
#==============
	...
	### couples as nodes
	for my $node (keys %{$self->{FAM}{PARENT_NODE}{$fam}}) {
		my ($fid, $mid) = @{$self->{FAM}{PARENT_NODE}{$fam}{$node}};

		### check for parent nodes
		for my $parent ($fid, $mid) {
		...
		for my $child (keys %{$self->{FAM}{CHILDREN_COUPLE}{$fam}{$fid}{$mid}}) {
			if (defined $self->{FAM}{CHILDREN_COUPLE}{$fam}{$child}) {
				for my $mate (keys %{$self->{FAM}{CHILDREN_COUPLE}{$fam}{$child}}) {
		...
		for my $parent (keys %{$self->{FAM}{CHILDREN_COUPLE}{$fam}}) {
		...
		for my $p (keys %path) {
			...
			if (defined $self->{FAM}{SID2FATHER}{$fam}{$pid1} && defined $self->{FAM}{SID2FATHER}{$fam}{$pid2}
				...
				if (($self->{FAM}{SID2FATHER}{$fam}{$pid1} eq $self->{FAM}{SID2FATHER}{$fam}{$pid2})
				    or ($self->{FAM}{SID2MOTHER}{$fam}{$pid1} eq $self->{FAM}{SID2MOTHER}{$fam}{$pid2})) {
		...
		for my $loop2 (keys %D1) {
			...
			if ($lp1) {
				...
				for my $k (keys %{$D1{$lp1}}) {
					$lflag = 1 unless $D1{$lp2}{$k};
				}
				$D2{$lp2} = 1 unless $lflag;
			}
		}
	}
	...
	for my $loop (keys %D) {
		...
		for my $couple (@loop_list) {
			for my $pid (split '==', $couple) {
				$E{$pid} = 1;
			}
		}
		...
		for my $node (@loop_list) {
			...
			for my $p (keys %P) {
				...
				if ($self->{FAM}{SID2FATHER}{$fam}{$p} && $E{$self->{FAM}{SID2FATHER}{$fam}{$p}}) {
					$no_start_flag = 1;
				}
				if ($self->{FAM}{SID2MOTHER}{$fam}{$p} && $E{$self->{FAM}{SID2MOTHER}{$fam}{$p}}) {
					$no_start_flag = 1;
				}
				...
				for my $p1 (keys %P) {
					for my $p2 (keys %P) {
						if ($self->{FAM}{CHILDREN_COUPLE}{$fam}{$p1}{$p2}) {
							for my $child (keys %{$self->{FAM}{CHILDREN_COUPLE}{$fam}{$p1}{$p2}}) {
								if ($E{$child}) {
									$no_end_flag = 1;
								}
							}
		...
		unless (defined $s->{NR2END}{$countl}) {
			my %R;
			for my $node (@loop_list) {
				for my $p (split '==', $node) {
					if ($R{$p}) {
						for my $mate (keys %{$self->{FAM}{COUPLE}{$fam}{$p}}) {
							if ($E{$mate}) {
								if ($self->{FAM}{SID2MOTHER}{$fam}{$mate} && $E{$self->{FAM}{SID2MOTHER}{$fam}{$mate}}
								    && $self->{FAM}{SID2FATHER}{$fam}{$mate} && $E{$self->{FAM}{SID2FATHER}{$fam}{$mate}}) {
	...
	my %skip;
	for my $loop1 (keys %{$s->{NR2END_UNIQUE}}) {
		my $c1 = scalar keys %{$s->{NR2END_UNIQUE}{$loop1}};
		for my $loop2 (keys %{$s->{NR2END_UNIQUE}}) {
			next if $loop1 == $loop2;
			my $c2 = scalar keys %{$s->{NR2END_UNIQUE}{$loop2}};
			if ($c1 > $c2) {
				my $cc = 0;
				for my $p (keys %{$s->{NR2END_UNIQUE}{$loop2}}) {
	...
	for my $loop (keys %{$s->{NR2END_UNIQUE}}) {
		next if $skip{$loop};
...
#========================
	...
	for my $fam (keys %{$self->{FAM}{HAPLO}}) {
		for my $pid (keys %{$self->{FAM}{HAPLO}{$fam}{PID}}) {
			if ($self->{FAM}{DUPLICATED_PID}{$fam}{$pid}) {
				for my $pid_n (keys %{$self->{FAM}{DUPLICATED_PID}{$fam}{$pid}}) {
...
#============
	...
	if ($arg{-format} eq '1') {
		...
		for (split "\n", ${$arg{-data}}) {
			...
			next unless ($chr && defined $pos && $marker);
			$map{POS}[$i] = $pos;
			$map{MARKER}[$i] = $marker;
			$i++;
		}
	...
	for my $fam (@fam) {
		...
		for my $pos (@{$map{POS}}) {
			push @{$self->{FAM}{MAP}{$fam}{POS}}, $pos;
		}
		for my $marker (@{$map{MARKER}}) {
			push @{$self->{FAM}{MAP}{$fam}{MARKER}}, $marker;
		}
	}
	1;
}
...
#============
	...
	ShowInfo("File $arg{-file} is empty!", 'warning') unless $file;
	...
	for my $fam (nsort keys %ped_org) {
		for (keys %{$self->{FAM}}) {
			delete $self->{FAM}{$_}{$fam} if defined $self->{FAM}{$_}{$fam};
		}
		AddFamToSelf($fam);
...
#============================
	...
	for my $fam (keys %$pedref) {
		for my $l (@{$pedref->{$fam}}) {
			...
			for (0 .. 5) {
				unless (defined $_[$_] && $_[$_] =~ /^$check{LINKAGE}{$_}$/) {
					for (@_[0 .. 5]) {
						$_ = '' unless defined $_;
					}
...
#==================
	...
	for my $l (@{$self->{FAM}{PED_ORG}{$fam}}) {
		...
		if ($fid && $mid) {
			...
			if ($fid eq $mid) {
				push @er, "Father and mother of individual $old_sid are identical!\n";
				next;
			}
			...
			push @er, "Error in line - father or mother must not be zero: @$l\n";
			next;
		}
		...
		if ($self->{FAM}{PID}{$fam}{$sid}) {
			push @er, "Individual $sid is duplicated!\n";
		} else {
		...
		if ($sample_info[4]) {
			$twins{$sample_info[4]}{$sid} = 1;
		}

		### storing consanguine information for later processing
		if ($sample_info[5]) {
			$consang{$sample_info[5]}{$sid} = 1;
		}
	...
	for my $sid (keys %{$self->{FAM}{SID2MOTHER}{$fam}}) {
		my $sid_old = $ci->{PID}{$sid}{Case_Info_1};
		my $mid = $self->{FAM}{SID2MOTHER}{$fam}{$sid};
		unless ($self->{FAM}{PID}{$fam}{$mid}) {
	...
	for my $founder (keys %{$self->{FAM}{FOUNDER}{$fam}}) {
	...
	if (%consang) {
		for my $k (keys %consang) {
			...
			unless ($self->{FAM}{CHILDREN_COUPLE}{$fam}{$cons[0]}{$cons[1]}) {
				push @er, "You declared individuals: $_ as consanguineous but they have no offspring!";
				next;
			}
...
#==================
	...
	return unless $self->{FAM}{HAPLO} && $self->{FAM}{HAPLO}{$fam} && keys %{$self->{FAM}{HAPLO}{$fam}{PID}};
	my %t;
	...
	for my $p (keys %{$self->{FAM}{PID}{$fam}}) {
		...
		for my $mp (qw/M P/) {
			for (@{$self->{FAM}{HAPLO}{$fam}{PID}{$p}{$mp}{BAR}}) {
...
#======================
	...
	while ($flag) {
		...
		for my $pid (keys %{$self->{FAM}{PID}{$fam}}) {
			...
			unless ($h->{$pid}{P}{BAR} && $h->{$pid}{M}{BAR}) {
				next unless $h->{$pid}{M}{TEXT} && $h->{$pid}{P}{TEXT};
				...
				if ($self->{FAM}{DUPLICATED_PID_ORIG}{$fam}{$pid}) {
					...
					if ($h->{$orig_pid}{P}{BAR} && $h->{$orig_pid}{M}{BAR}) {
						for (@{$h->{$orig_pid}{P}{BAR}}) {
							push @{$h->{$pid}{P}{BAR}}, [ @$_ ];
						}
						for (@{$h->{$orig_pid}{M}{BAR}}) {
							push @{$h->{$pid}{M}{BAR}}, [ @$_ ];
						}
					}
					next;
	...
	for my $partner (keys %{$self->{FAM}{SIBS}{$fam}}) {
		...
		for my $s (keys %P) {
			foreach	(keys %{$self->{FAM}{COUPLE}{$fam}{$s}}) {
				unless ($self->{FAM}{FOUNDER}{$fam}{$_} || $self->{FAM}{CHILDREN}{$fam}{$s}{$_} || $self->{FAM}{DUPLICATED_PID_ORIG}{$fam}{$_}) {
					next P;
				}
			}
		}
		...
		if (
		    (defined $self->{FAM}{FOUNDER}{$fam}{$f} || defined $self->{FAM}{DUPLICATED_PID_ORIG}{$fam}{$f})
		    && (defined $self->{FAM}{FOUNDER}{$fam}{$m} || defined $self->{FAM}{DUPLICATED_PID_ORIG}{$fam}{$m})
		   ) {
			my @TOP = ($f, $m);
			ChangeOrder(\@TOP) unless $param->{SORT_COUPLE_BY_GENDER};
			$Top{$partner} = [ @TOP ];
			$self->{FAM}{STRUK}{$fam} = [
						     [
						      [
...
#================
	...
	if ($param->{SORT_BY_PEDID}) {
		...
		@$array = ();
		for (nsort keys %s) {
			push @$array, $self->{FAM}{PID2PIDNEW}{$fam}{$_};
		}
	} else {
		### mix this array
		for (my $i = @$array; --$i;) {
...
#								my $c = join '==', nsort @{$r->[0]};
								...
								my $founder = 0;
								for my $coupl (@{$r->[0]}) {
									unless ($self->{FAM}{FOUNDER}{$fam}{$coupl}) {
										$founder++;
									}
								}
								...
								if (!$self->{GLOB}{STRUK_MODE} && ($founder > 1)) {
									if ($skip->{$child} && ($skip->{$child} == ($founder - 1))) {
										push @children, $child;
									} else {
										$skip->{$_}++ for @{$r->[0]};
									}
								} else {
									push @children, $child unless $skip2->{$child};
									$skip2->{$_}++ for @{$r->[0]};
								}
							} else {
								push @children, $child;
...
#======================
	...
	$fb->Button(-text => 'Connect', -width => 10, -command => sub {
		...
		### Performing $self and preprocessing families
		for my $fam (nsort keys %ped_org) {
			for (keys %{$self->{FAM}}) {
				delete $self->{FAM}{$_}{$fam} if $self->{FAM}{$_}{$fam};
			}
			AddFamToSelf($fam);
			$self->{FAM}{PED_ORG}{$fam} = $ped_org{$fam};
			ProcessFamily($fam) or return;
	...
	### Cancel bottom
	$fb->Button(-text => 'Cancel', -width => 10,
		    -command => sub {
			    $d->destroy();
	...
	$f1->Label(-text => 'Database type', -width => 13, -justify => 'left', -anchor => 'w',)->pack(qw/-side left/);
	$f1->BrowseEntry(-textvariable => \$self->{GLOB}{DB_TYPE}, -choices => [ qw/Oracle PostgreSQL MySQL/ ],
	...
	$f2->Label(-text => 'Hostname', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	my $entry2 = $f2->Entry(-textvariable => \$self->{GLOB}{DB_HOST}, -width => 25)->pack(qw/-side left/);

	$f3->Label(-text => 'Port', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	$f3->Entry(-textvariable => \$self->{GLOB}{DB_PORT}, -width => 25)->pack(qw/-side left/);

	$f4->Label(-text => 'Table name', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	$f4->Entry(-textvariable => \$self->{GLOB}{DB_RELATION}, -width => 25)->pack(qw/-side left/);

	$f5->Label(-text => 'DBname(SID)', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	$f5->Entry(-textvariable => \$self->{GLOB}{DB_SID}, -width => 25)->pack(qw/-side left/);

	$f6->Label(-text => 'Username', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	$f6->Entry(-textvariable => \$self->{GLOB}{DB_UNAME}, -width => 25)->pack(qw/-side left/);

	$f7->Label(-text => 'Password', -width => 15, -justify => 'left', -anchor => 'w')->pack(qw/-side left/);
	$f7->Entry(-textvariable => \$self->{GLOB}{DB_PASSWD}, -show => '*', -width => 25)->pack(qw/-side left/);
...
#=================
	...
	my $be3 = $f->BrowseEntry(-label => 'Orientation: ', -variable => \$self->{GLOB}{ORIENTATION},
				  -width => 15, -choices => [ qw/Landscape Portrait/ ],
				  -labelPack => [ -side => 'top', -anchor => 'w' ])->grid(-row => 3, -column => 0, -sticky => 'w', -padx => 4, -pady => 10);

	my $be4 = $f->BrowseEntry(-label => 'Paper Size: ', -variable => \$self->{GLOB}{PAPER},
				  -width => 15, -choices => [ nsort keys %{$param->{PAPER_SIZE}} ],
				  -labelPack => [ -side => 'top', -anchor => 'w' ])->grid(-row => 3, -column => 1, -sticky => 'w', -padx => 4, -pady => 10);
...
#=======================
	...
	### Buttons on bottom
	$f2->Button(-text => 'Ok', -width => 10,
		    -command => sub {
			    RedrawPed();
			    $d->destroy;
		    })->grid(-row => 0, -column => 0, -sticky => 'w');

	$f2->Button(-text => 'Cancel', -width => 10,
		    -command => sub {
			    $self = thaw($freeze);
			    $d->destroy;
			    RedrawPed();
	...
	$f2->Button(-text => 'Preview', -width => 10,
		    -command => sub {
			    RedrawPed();
		    })->grid(-row => 0, -column => 3, -sticky => 'w');

	### choose PIDs for Loop break
	$f1->Label(-text => 'Select consanguineous couples', -width => 30)->pack(-side => 'top', -anchor => 'n');
	my $lb = $f1->Scrolled('Listbox',
			       -scrollbars => 'osoe', -selectmode => 'extended', -selectbackground => 'red',
			       -height => 14, -width => 25, -exportselection => 0)->pack(-side => 'top', -fill => 'both', -expand => 1);
...
#=====================
	...
	$f3->Button(-text => 'Cancel', -width => 10,
		    -command => sub {
			    $self = thaw($freeze);
			    $d->destroy;
		    })->grid(-row => 0, -column => 1, -sticky => 'w');

	$f3->Button(-text => 'Preview', -width => 10,
		    -command => sub {
			    for my $k (keys %{$self->{FAM}}) {
				    undef $self->{FAM}{$k}{$fam} unless defined $def->{FAM}{$k};
			    }
	...
	if ($self->{FAM}{LOOP_BREAK_STATUS}{$fam} == 3) {
		for (my $i = 0; $i < $nr_loops; $i++) {
			...
			$p2 = $self->{FAM}{PID2PIDNEW}{$fam}{$p2};
			$_ = join '==', nsort($p1, $p2);
			if ($self->{FAM}{BREAK_LOOP_OK}{$fam}{$_} == 1) {
				$lb6->selectionSet($i);
			} else {
				$lb6->selectionClear($i);
			}
...
#==================
	...
	### Notebook
	my $n = $f1->NoteBook(-background => '#28D0F0')->pack(-expand => 1, -fill => 'both');
	...
	for my $s (
		       ...
		       [ 'BBOX_WIDTH',         'Width of bounding boxes',      3, 1,   10, 100,   1 ],
		       [ 'LEGEND_SHIFT_RIGHT', 'Legend distance right',        4, 1,   20, 500,   5 ],
		      ) {
		$p1_f1->Scale(-label => $s->[1], -variable => \$self->{FAM}{$s->[0]}{$fam},
			      -from => $s->[4], -to => $s->[5], -orient => 'horizontal', -length => 160, -width => 12,
			      -resolution => $s->[6])->grid(-row => $s->[2], -column => $s->[3], -sticky => 'ns');
	...
	$fc5->Button(-text => 'Color of all haplotypes', -width => 20, -height => 1,
		     -command => sub {
			     ...
			     for my $p (keys %{$self->{FAM}{FOUNDER}{$fam}}) {
				     next unless $self->{FAM}{HAPLO}{$fam}{PID}{$p};
				     for my $mp (qw/M P/) {
	...
	my $mb = $fc4->Button(-text => 'Color of maternal haplotype', -width => 25, -height => 1,
			      -command => sub {
				      #print "PID:$pid\n";
	...
	my $cbs1 = $p3_f1->Checkbutton(-text => 'Anchor paternal haplotype')->grid(-row => 3, -column => 2, -sticky => 'w');
	my $cbs2 = $p3_f1->Checkbutton(-text => 'Anchor maternal haplotype')->grid(-row => 4, -column => 2, -sticky => 'w');

	my $cbs3 = $p3_f1->Checkbutton(-text => 'Hide paternal haplotype')->grid(-row => 5, -column => 2, -sticky => 'w');
	my $cbs4 = $p3_f1->Checkbutton(-text => 'Hide maternal haplotype')->grid(-row => 6, -column => 2, -sticky => 'w');

	### personenbezogene Einstellungen
	@_ = nsort keys %{$self->{FAM}{FOUNDER}{$fam}};
	for (@_) {
		$_ = $self->{FAM}{CASE_INFO}{$fam}{PID}{$_}{'Case_Info_1'};
	}
	my $be5 = $p3_f1->BrowseEntry(-label => '        Founder:', -variable => \$pid_old,
				      ...
				      -browsecmd => sub {
					      ...
					      $pb->configure(-state => $_);
					      if ($self->{FAM}{HAPLO}{$fam}{PID}{$pid}{M}{SAVE}) {
						      $_ = 'disabled';
					      } else {
						      $_ = 'normal';
					      }
					      $mb->configure(-state => $_);
					      $cbs1->configure(-variable => \$self->{FAM}{HAPLO}{$fam}{PID}{$pid}{P}{SAVE},
							       -command => sub {
								       if ($self->{FAM}{HAPLO}{$fam}{PID}{$pid}{P}{SAVE}) {
					      ...
					      $cbs2->configure(-variable => \$self->{FAM}{HAPLO}{$fam}{PID}{$pid}{M}{SAVE},
							       -command => sub {
								       if ($self->{FAM}{HAPLO}{$fam}{PID}{$pid}{M}{SAVE}) {
	...
	for (0 .. 8) {
		$p3_f1->gridRowconfigure($_, -pad => 10);
	}
	...
	if ($self->{FAM}{MAP}{$fam}{MARKER}) {
		...
		for (my $i = 0; $i < scalar @_; $i++) {
			my $j = '';
			unless (defined $self->{FAM}{MAP}{$fam}{POS}[$i]) {
				$j = sprintf("%03.0f - ", $i + 1);
			} else {
	...
	my $be6 = $p4_f1->BrowseEntry(-label => 'Sample', -variable => \$person,
	...
	$lb6->bind('<ButtonRelease-1>' => sub {
			   if ($self->{FAM}{MAP}{$fam}{MARKER}) {
				   ...
				   for (my $i = 0; $i < scalar @{$self->{FAM}{MAP}{$fam}{MARKER}}; $i++) {
					   if ($h{$i}) {
						   $self->{FAM}{HAPLO}{$fam}{PID}{$sid_new}{BOX}[$i] = 1;
					   } else {
						   $self->{FAM}{HAPLO}{$fam}{PID}{$sid_new}{BOX}[$i] = 0;
					   }
				   }
			   }
		   });
	...
	for my $nr (sort { $a <=> $b } keys %{$c{LC}}) {
		...
		if ($nr == 1) {
			$f->Checkbutton(-text => "Set for affected",
					-variable => \$self->{FAM}{SYMBOL_LINE_COLOR_SET}{$fam})->pack(qw/-side left/);
		}
	}
	...
	$f->Checkbutton(-text => "Export background",
			-variable => \$self->{FAM}{EXPORT_BACKGROUND}{$fam})->pack(qw/-side left/);

	for (0 .. 9) {
		$p6_f1->gridRowconfigure($_, -pad => 2);
	}
	...
	$p7_f1->Checkbutton(-text => "Show",
			    -variable => \$self->{FAM}{SHOW_HEAD}{$fam})->grid(-row => 0, -column => 0, -sticky => 'e', -pady => 10);

	$p7_f1->Entry(-textvariable => \$self->{FAM}{TITLE}{$fam},
		      -width => 40)->grid(-row => 0, -column => 1, -sticky => 'w');

	$p7_f1->BrowseEntry(label => 'Font size: ', -variable => \$self->{FAM}{FONT_HEAD}{$fam}{SIZE},
			    -command => sub {}, -width => 5, -choices => [ 5 .. 20, 22, 24, 26, 28, 36, 48, 72 ],
			    -state => 'readonly')->grid(-row => 0, -column => 2, -sticky => 'w');

	for my $col (1 .. 5) {
		$p7_f3->BrowseEntry(-label => 'Case  ' . $col, -labelPack => [ -side => 'left', -anchor => 'w' ],
		...
		$p7_f3->Checkbutton(-text => 'Show',
				    -variable => \$self->{FAM}{CASE_INFO_SHOW}{$fam}{$col})->grid(-row => $col, -column => 1, -sticky => 's');
	}
...
#===============
	...
	my $be1 = $fe1->BrowseEntry(-variable => \$fo->{FAMILY}, -state => 'readonly',
				    -choices => [ nsort $mw->fontFamilies, 'Lucida' ],
				    -command => sub {
					    $cb1->configure(-font => [ $fo->{FAMILY}, 8, $fo->{WEIGHT}, $fo->{SLANT} ]);
				    })->pack(qw/-side left/);
	...
	my $be2 = $fe2->BrowseEntry(-variable => \$fo->{SIZE}, -state => 'readonly',
				    -choices => [ 5 .. 20, 22, 24, 26, 28, 30, 32, 36, 40, 44, 48, 56, 64, 72 ])->pack(qw/-side left/);
	...
	my $be4 = $fe4->BrowseEntry(-variable => \$fo->{SLANT}, -state => 'readonly', -choices => [ qw/italic roman/ ],
				    -command => sub {
					    $cb1->configure(-font => [ $fo->{FAMILY}, 10, $fo->{WEIGHT}, $fo->{SLANT} ]);
				    })->pack(qw/-side left/);

	### Font Color
	my $fc1 = $f1->Frame->grid(-row => 5, -column => 1, -sticky => 'w');
...
#===============
	...
	if (!$arg{-fit}) {
		...
		if ($ybd && $wy) {
			if ($xbd / $ybd > $wx / $wy) {
				$self->{FAM}{ZOOM}{$fam} *= $wx / $xbd * 0.9;
			} else {
				$self->{FAM}{ZOOM}{$fam} *= $wy / $ybd * 0.9;
			}
		}
...
#=======================
	...
	if (!$arg{-modus}) {
		...
		for my $parent_node (keys %{$d->{COUPLE}}) {
			for my $ln (@{$d->{COUPLE}{$parent_node}}) {
				$canvas->createLine(@$ln, -width => $l * $z, -fill => $lnc);
			}
		}
		for my $id (keys %{$d->{SIB}}) {
			for my $ln (@{$d->{SIB}{$id}}) {
				$canvas->createLine(@$ln, -width => $l * $z, -fill => $lnc);
			}
		}
		for my $id (keys %{$d->{COUPLE_SIB}}) {
			for my $ln (@{$d->{COUPLE_SIB}{$id}}) {
				$canvas->createLine(@$ln, -width => $l * $z, -fill => $lnc);
			}
		}
		for my $id (keys %{$d->{TWIN_LINES}}) {
			for my $ln (@{$d->{TWIN_LINES}{$id}}) {
				$canvas->createLine(@$ln, -width => $l * $z, -fill => $lnc);
			}
		}

		### Drawing male SYMBOL Elements
		for my $r (@{$de->{SYM_MALE}}) {
			$canvas->createRectangle(@$r);
		}
		### Drawing female SYMBOL Elements
		for my $r (@{$de->{SYM_FEMALE}}) {
			$canvas->createOval(@$r);
		}
		### Drawing stillbirth SYMBOL Elements
		for my $r (@{$de->{SYM_STILLBIRTH}}) {
			$canvas->createPolygon(@$r);
		}
		### Drawing unknown SYMBOL Elements
		for my $r (@{$de->{SYM_UNKNOWN}}) {
			$canvas->createPolygon(@$r);
		}
		#### Drawing Text elements
		for my $e (qw/INNER_SYMBOL_TEXT CASE_INFO TITLE PROBAND_TEXT MARK_TEXT SAB_GENDER/) {
			for my $r (@{$de->{$e}}) {
				$canvas->createText(@$r);
			}
		}
		### Drawing live status line
		for my $r (@{$de->{LIVE_LINE}}) {
			$canvas->createLine(@$r);
		}
		### Haplotype Bar not informative
		for my $r (@{$de->{HAP_BAR_NI}}) {
			$canvas->createRectangle(@$r);
		}
		### Haplotype Bar
		for my $r (@{$de->{HAP_BAR}}) {
		...
		### Haplotype Text
		for my $e (qw/HAP_TEXT MAP_MARKER_LEFT MAP_MARKER_RIGHT MAP_POS_LEFT MAP_POS_RIGHT/) {
			for my $r (@{$de->{$e}}) {
				$canvas->createText(@$r);
			}
		}
		...
		### Adopted lines
		for my $r (@{$de->{IS_ADOPTED}}) {
			$canvas->createLine(@$r);
		}

		#for my $r (@{$self->{FAM}{CROSS_CHECK}{$fam}}) {
		#	$canvas->createOval(@$r);
		#}
		...
		my @fams;
		if ($arg{-fammode}) {
			@fams = nsort keys %{$self->{FAM}{PED_ORG}};
		} else {
			@fams = $fam;
		}
		return unless @fams;
		...
		### write byte order mark (BOM) for Unicode encoded files
		if (($param->{ENCODING} ne 'ascii') && $param->{WRITE_BOM}) {
			print FH "\x{FEFF}";
		}
		print FH "$head\n";
		...
		### any text
		for my $e (qw/SAB_GENDER MARK_TEXT HAP_TEXT MAP_MARKER_LEFT MAP_MARKER_RIGHT MAP_POS_LEFT MAP_POS_RIGHT TITLE CASE_INFO INNER_SYMBOL_TEXT PROBAND_TEXT/) {
			for my $r (@{$de->{$e}}) {
		...
		### create different surfaces for different output formats
		if ($arg{-modus} eq 'PNG') {
			$surface = Cairo::ImageSurface->create('argb32', $x_pap_pix, $y_pap_pix);
			...
			ShowInfo("Unknown file format $arg{-modus}\n");
			return;
		}
		...
		for my $e (qw/SAB_GENDER MARK_TEXT HAP_TEXT MAP_MARKER_LEFT MAP_MARKER_RIGHT MAP_POS_LEFT MAP_POS_RIGHT TITLE CASE_INFO INNER_SYMBOL_TEXT PROBAND_TEXT/) {
			for my $r (@{$de->{$e}}) {
				...
				### workaround
				if ($r->[3] eq 'w') {
					$cairo->move_to($x, $y - ($height / 2));
				} elsif ($r->[3] eq 'e') {
					$cairo->move_to($x - $width, $y - ($height / 2));
				} elsif ($r->[3] eq 'center') {
					$cairo->move_to($x - ($width / 2), $y - ($height / 2));
				}
		...
		for my $r (@{$de->{ARROWS}}) {
			...
			### drawing arrow shape using trigonometric functions and the assumption
			### that the arrow angle being 45 degree
		...
		### PNG slightly differs in syntax --> extra command
		if ($arg{-modus} eq 'PNG') {
...
#===============
	...
	### adapt Y axis of drawing elements dependent of haplotypes and drawing style
	CanvasTrimYdim();

	### Title
	unless ($self->{FAM}{TITLE_X}{$fam}) {
	...
	if ($self->{FAM}{SHOW_HEAD}{$fam}) {
		unless ($self->{FAM}{TITLE}{$fam}) {
			$self->{FAM}{TITLE}{$fam} = "Family $fam";
		}
		push @{$de->{TITLE}}, [
				       $self->{FAM}{TITLE_X}{$fam} * $self->{FAM}{GITTER_X}{$fam} * $z,
				       $self->{FAM}{TITLE_Y}{$fam} * $self->{FAM}{GITTER_Y}{$fam} * $z,
				       -anchor => 'center', -text => $self->{FAM}{TITLE}{$fam},
				       -font => $head1, -fill => $self->{FAM}{FONT_HEAD}{$fam}{COLOR},
				       -tags => [ qw/TEXT HEAD TAG/ ]
				      ];
	}

	### Zeichnen aller Personen-bezogenen Elemente
	### Drawing of individual related elements
	for my $Y (keys %{$m->{YX2P}}) {
		for my $X (keys %{$m->{YX2P}{$Y}}) {
			my $p = $m->{YX2P}{$Y}{$X};
			if ($save{$p}) {
				next;
			}
			...
			if ($self->{FAM}{IS_SAB_OR_TOP}{$fam}{$p}) {
				...
				push @{$de->{SYM_STILLBIRTH}}, [
								($cx - $sz) * $z, $cy * $z, $cx * $z, ($cy - $sz) * $z, ($cx + $sz) * $z, $cy * $z,
								-width => $l * $z, -outline => $slnc_new, -fill => $col,
								-tags => [ 'SYMBOL', "SYM-$p", 'TAG' ]
							       ];
			} elsif ($sex == 1) {
				### male
				push @{$de->{SYM_MALE}}, [
							  ($cx - $sz) * $z, ($cy - $sz) * $z, ($cx + $sz) * $z, ($cy + $sz) * $z,
							  -width => $l * $z, -outline => $slnc_new, -fill => $col,
							  -tags => [ 'SYMBOL', "SYM-$p", 'TAG' ]
							 ];
			} elsif ($sex == 2) {
				...
				### unknown gender
				push @{$de->{SYM_UNKNOWN}}, [
							     ($cx - $sz * sqrt(2)) * $z, $cy * $z, $cx * $z, ($cy - $sz * sqrt(2)) * $z,
							     ($cx + $sz * sqrt(2)) * $z, $cy * $z, $cx * $z, ($cy + $sz * sqrt(2)) * $z,
							     -width => $l * $z, -outline => $slnc_new, -fill => $col,
							     -tags => [ 'SYMBOL', "SYM-$p", 'TAG' ]
							    ];
			...
			if ($self->{FAM}{SHOW_INNER_SYBOL_TEXT}{$fam} && defined $self->{FAM}{INNER_SYMBOL_TEXT}{$fam}{$p}
				...
				push @{$de->{INNER_SYMBOL_TEXT}}, [
								   ...
								   -font => $font2, -fill => $self->{FAM}{FONT_INNER_SYMBOL}{$fam}{COLOR},
								   -tags => [ 'TEXT', "INNER_SYMBOL_TEXT-$p", 'INNER_SYMBOL_TEXT' ]
								  ];
			}
...
#					my $y_pid = sprintf("%0.0f", $yp + ($f1->{SIZE} * $z) / 2);
			...
			if ($self->{FAM}{IS_PROBAND}{$fam}{$p}) {
				...
				push @{$de->{ARROWS}}, [
							$x2 * $z, $y2 * $z, $x1 * $z, $y1 * $z, -width => $plw * $z, -fill => $plc,
							-arrow => 'last', -arrowshape => [ $dist1 * $z, $dist2 * $z, $dist3 * $z ]
				];
			...
			if (defined $self->{FAM}{SIDE_SYMBOL_TEXT}{$fam}{$p}) {
				push @{$de->{MARK_TEXT}}, [
							   ($cx - $sz - ($f5->{SIZE} / 1.5)) * $z, ($cy - $sz - ($f5->{SIZE} / 1.5)) * $z,
							   -anchor => 'center', -text => $self->{FAM}{SIDE_SYMBOL_TEXT}{$fam}{$p},
							   -font => $font5, -fill => $f5->{COLOR}
							  ];
			}
...
#=============
	...
	my @X_GLOB;
	push @X_GLOB, (keys %{$m->{YX2P}{$_}}) for keys %{$m->{YX2P}};
	...
	my $ccc = 1;
	for (5, 4, 3, 2, 1) {
		if ($self->{FAM}{CASE_INFO_SHOW}{$fam}{$_}) {
			$ccc = $_ + 1;
			last;
		}
	}
		...
		for my $X (@X) {
			...
			if ($h->{PID}{$p}{P}{TEXT}) {
				if ($self->{FAM}{SHOW_HAPLO_BAR}{$fam}) {
					...
					for my $PM (qw/P M/) {
						...
						for (my $i = 0; $i <= $i2; $i++) {
							...
							undef $fill unless $self->{FAM}{FILL_HAPLO}{$fam};
							...
							if ($self->{FAM}{HAPLO_SEP_BL}{$fam}) {
								...
								if ($i != $i2) {
									...
									my $i4;
									for ($i4 = $i + 1; $i4 <= $i2; $i4++) {
										last if $self->{FAM}{HAPLO}{$fam}{DRAW}[$i4];
									}
				...
				if ($self->{FAM}{SHOW_HAPLO_BBOX}{$fam} && $h->{PID}{$p}{BOX}) {
					...
					my $cc = 0;
					my $td = $td1;
					unless ($self->{FAM}{SHOW_HAPLO_TEXT}{$fam}) {
...
#=============
	...
	for my $P (keys %{$self->{FAM}{PID_SAVE}{$fam}}) {
		next if $pid eq $P;
		next unless $m->{P2XY}{$P}{Y} == $Y;
		my $X = $m->{P2XY}{$P}{X};
...
#=============
	...
	for my $parent_node (keys %{$self->{FAM}{PARENT_NODE}{$fam}}) {
		...
		if (($self->{FAM}{LOOP}{$fam}{CONSANGUINE}{$par1} && $self->{FAM}{LOOP}{$fam}{CONSANGUINE}{$par1}{$par2})
			...
			if ($ym1 == $ym2) {
				...
				### otherwise draw three double lines dependent on Y-axis location in two ways
				$f *= -1 if $x1 > $x2;
				$d->{COUPLE}{$parent_node} = [
							      [ $x1, $ym1 - $cd * $z, $x1 + $rd + $f,
								$ym1 - $cd * $z, $x1 + $rd + $f,
								$ym2 - $cd * $z, $x2, $ym2 - $cd * $z ],
							      [ $x1, $ym1 + $cd * $z, $x1 + $rd,
								$ym1 + $cd * $z, $x1 + $rd,
								$ym2 + $cd * $z, $x2, $ym2 + $cd * $z ]
							     ];
			...
			if ($ym1 == $ym2) {
				...
				$d->{COUPLE}{$parent_node} = [ [ $x1, $ym1, $x1 + $rd, $ym1, $x1 + $rd, $ym2, $x2, $ym2 ] ];
				push @{$d->{LINE_CROSS}}, (
							   [ $x1, $ym1, $x1 + $rd, $ym1 ],
							   [ $x1 + $rd, $ym1, $x1 + $rd, $ym2 ],
							   [ $x1 + $rd, $ym2, $x2, $ym2 ]
				...
				$d->{COUPLE}{$parent_node} = [ [ $x2, $ym2, $x2 - $rd, $ym2, $x2 - $rd, $ym1, $x1, $ym1 ] ];
				push @{$d->{LINE_CROSS}}, (
							   [ $x2, $ym2, $x2 - $rd, $ym2 ],
							   [ $x2 - $rd, $ym2, $x2 - $rd, $ym1 ],
							   [ $x2 - $rd, $ym1, $x1, $ym1 ]
	...
	for my $parent_node (keys %{$d->{COUPLE}}) {
		...
		if (scalar @children > 1) {
			...
			for my $child_x ($K_F, @child_x, $K_L) {
				### vertical lines for cross check
			...
			### save single children for later queries
			if (ref $children[0]) {
				### code for twins here
	...
	for my $parent_node (keys %{$d->{COUPLE}}) {
		...
		if ($d->{SIB}{$parent_node}) {
			...
			if (($x4 <= $xm1) || ($x3 >= $xm1)) {
				...
				### direct connection depending on group width
				if ($xd1 <= $xd2) {
			...
			if (ref $d->{CHILD}{$parent_node}[0]) {
				...
				if (($x1 < $x3) && ($x2 > $x3)) {
					...
					### split
					$d->{COUPLE_SIB}{$parent_node} = [ [ $xm1, $y1, $xm1, $y3, $x3, $y3, $x3, $y4 ] ];
					push @{$d->{LINE_CROSS}}, ([ $xm1, $y1, $xm1, $y3 ], [ $xm1, $y3, $x3, $y3 ], [ $x3, $y3, $x3, $y4 ]);
				}
...
#==================
	...
	unless (defined $X && defined $Y) {
		die "GetCanvasCoor error at pid: $p", Dumper($self->{STRUK});
	}
	my ($cx, $cy) = ($X * $self->{FAM}{GITTER_X}{$fam}, $Y * $self->{FAM}{GITTER_Y}{$fam});
	return (($cx - $sz) * $z, ($cy - $sz) * $z, ($cx + $sz) * $z, ($cy + $sz) * $z);
}

### new algorithm for detecting parallel or crossed line intersection 07/2008
#===============
sub CrossCheck {
#===============
	my ($fam, $r1, $r2) = @_;
	...
	if ($rd == 0 && $rn == 0) {
		if ($ax == $bx) {
			...
			if ((($cy >= $ay) && ($cy < $by)) || (($ay >= $cy) && ($ay < $dy))) {
				push @{$self->{FAM}{CROSS_CHECK}{$fam}}, [ $ax - (5 * $z), $ay - (5 * $z), $ax + (5 * $z), $ay + (5 * $z),
									   -width => 1, -outline => '#c0c0c0', -fill => '#000000' ];
				return 1;
			}
		} else {
	...
	push @{$self->{FAM}{CROSS_CHECK}{$fam}}, [ $intersection_x - (5 * $z), $intersection_y - (5 * $z), $intersection_x + (5 * $z), $intersection_y + (5 * $z),
						   -width => 1, -outline => '#000000', -fill => '#c0c0c0' ];
	return 1;
}
...
#===============
	...
	## find everybody joined in couple group
	my %P;
	for (keys %{$self->{FAM}{COUPLE}{$fam}{$child}}) {
		$P{$_} = 1 unless $self->{FAM}{CHILDREN}{$fam}{$child}{$_};
	}
	my $flag = 1;
	while ($flag) {
		$flag = 0;
	...
	if ($param->{SORT_COUPLE_BY_GENDER}) {
		@_ = ();
		for (@S) {
			push @_, $_ if $self->{FAM}{SID2SEX}{$fam}{$_} == 1;
		}
		for (@S) {
			push @_, $_ if $self->{FAM}{SID2SEX}{$fam}{$_} != 1;
		}
	...
	### additionally store mate order in respect of centering children correct
	my @D2;
	if ($#S > 1) {
		for my $i (0 .. $#S - 1) {
			push @D2, [ $S[$i], $S[$i + 1] ];
...
#=================
	...
	for (@children) {
		...
		if ($self->{FAM}{SID2SEX}{$fam}{$p} == 1) {
			$self->{FAM}{SID2FATHER}{$fam}{$_} = $pn;
		} else {
			$self->{FAM}{SID2MOTHER}{$fam}{$_} = $pn;
		}
	...
	### the new person is founder per se
...
#==============
	...
	for my $p (keys %{$self->{FAM}{LOOP}{$fam}{BREAK}}) {
		for my $mate (keys %{$self->{FAM}{LOOP}{$fam}{BREAK}{$p}}) {
